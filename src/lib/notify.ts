...
async function emailNotify(newReleases: NewRelease[], s: Settings): Promise<void> {
...
export async function notifyNewReleases(newReleases: NewRelease[]): Promise<void> {
...
export async function sendTestEmail(override?: Settings): Promise<void> {
  ...
  if (!smtpIsConfigured(s)) {
    throw new Error("SMTP is not configured (host, user, and recipient required).");
  }
  await transport(s).sendMail({
    from: s.smtp.from || s.smtp.user,
    to: s.smtp.to,
